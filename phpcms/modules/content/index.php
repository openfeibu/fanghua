...
class index {
	...
	public function init() {
		...
		if($_SERVER['SERVER_NAME'] == 'm.fanghualawyer.com'){
	...
	protected function _category_priv($catid) {
	 ...
	 public function  theajax()
	 {
		...
		$data=$this->db->select($where,"*","$page,10","id DESC");
		$text="";
		foreach( $data as $key => $value )
		{
			$time=date('Y-m-d',$value['inputtime']);
			$url=str_replace("www","m",$value['url']);
			$text.="<li><a href=".$url."><p>".$value['title']."</p><span>".$time."</span></a></li>	";
		}
		echo $text;
	 }
}
